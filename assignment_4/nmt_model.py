...
-1) * target_masks[1:]
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded) # embed the padded source         
        X = self.post_embed_cnn(X.permute(1, 2, 0)) # reshape or the CNN, which uses channels instead of LSTM input parameters                 
        X = pack_padded_sequence(X.permute((2,0,1)), source_lengths) # unshape the tensor back to LSTM order, pack to prevent the RNN processing padding tokens
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)  # enc_hiddens is the hiden state for every time step, last_hidden is the final hidden state, last_cell is the final hidden cell state   
        enc_hiddens, _ = pad_packed_sequence(sequence=enc_hiddens) #add the pading back in after the RNN, _ is meassure of each seq length that is unused 
        enc_hiddens = enc_hiddens.permute((1,0,2))  #(src_len, b, h*2) is returned, we want (b, src_len, h*2)               
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), 1)  #combines forward and backward passes 
        init_decoder_hidden = self.h_projection(last_hidden)  # project back down to decoder hidden size      
        last_cell = torch.cat((last_cell[0], last_cell[1]), 1) # do the same for the hidden cell state       
        init_decoder_cell = self.c_projection(last_cell) #  do the same for the hidden cell state              
        dec_init_state = (init_decoder_hidden, init_decoder_cell) # init the decoder state
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens)  # attention needs to be projected down to hidden size from 2h
        Y = self.model_embeddings.target(target_padded)      # Embed target sentence 
        for Y_t in torch.split(Y, split_size_or_sections=1, dim=0):  #split target setences into subwords 
            Y_t = Y_t.squeeze(dim=0)                         # Split is counterintutive, it doesn't return a list of vectors, it returns a list of tenors with one redundant dimension... 
            Ybar_t = torch.cat((Y_t, o_prev), dim=-1)        # dim-1 indicates last dimension, resulting in adding h to e for the second dimenion 
            dec_state, combined_output, _, = self.step(      # do an RNN step on the decoder 
                Ybar_t=Ybar_t,                  
                dec_state=dec_state, 
            ...
            combined_outputs.append(combined_output) # keep a record of all the outputs
            o_prev = combined_output #make the output of this part of the next step
        combined_outputs = torch.stack(combined_outputs)     # concatenate outputs along a new dimension, creating a tensor of all the previous steps
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ## dec_hidden is shape (b, h)
        ## enc_hiddens_proj is shape (b, src_len, h)

        dec_state = self.decoder(Ybar_t, dec_state)   # send the current target and decoder state to the decoder cell                          
        (dec_hidden, dec_cell) = dec_state            # split decstate into a tupple 
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2) # do a matrix multiplication across all batches 
        ...
        a_t = torch.einsum("bl, blh->bh", alpha_t, enc_hiddens)          # (b, 2h)
